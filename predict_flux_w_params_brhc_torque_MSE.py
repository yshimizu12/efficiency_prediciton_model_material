...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        id_, iq_ = self.labels['current'][index]
        ...
        return (
            ...
            torch.tensor(id_, dtype=torch.float32),
            torch.tensor(iq_, dtype=torch.float32),
...
df_sp = pd.read_csv(
    '/sqfs2/cmc/1/work/G15489/v60716/_data/data_bmwi3/dataset_scaling_parameter_all.csv'
)
df_sp.index = ['mean','std']
def scaling(x, col):
    return (x-df_sp.loc['mean',col])/df_sp.loc['std',col]
def unscaling(x, col):

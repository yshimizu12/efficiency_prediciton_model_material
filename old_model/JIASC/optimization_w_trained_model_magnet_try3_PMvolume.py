...
print(float((generated_image_.max(axis=1)[1]==2).sum())/(256*256)*100)
...
class Optimize:
    ...
    def only_setting(self, Ie_max, Vdc, TEMP_PM, PM_material, torques, efficiencies):
        self.params_prediction['Ie_max'] = float(Ie_max)
        self.params_prediction['Vdc'] = float(Vdc)
        self.params_prediction['TEMP_PM'] = float(TEMP_PM)
        self.params_prediction['PM_material'] = PM_material
...
# hoge = np.array(f_1).reshape(100*50,2)
# plt.plot(hoge[:,0],hoge[:,1],'o')
# # plt.plot(f_1[50][:,0],f_1[50][:,1],'rs')
# # plt.plot(f_1[100][:,0],f_1[100][:,1],'gs')
# # plt.plot(f_1[150][:,0],f_1[150][:,1],'cs')
# plt.plot(f_1[-1][:,0],f_1[-1][:,1],'rs')
...
save_dir = Path('optimization_results') / 'change_PM'
dt = '230529194515'
df_name = '_magnet_volume'
# x_1_all = []
pf_tmp_all = {}

f.only_setting(
    Ie_max=300, Vdc=650, TEMP_PM=60, PM_material='R30',
    torques=[[280, 3000],[55, 11000]], 
...
for PM_mat in PM_material_list:
    for PM_temp in PM_TEMP_list:
        print(f"mat_{PM_mat}_temp_{PM_temp}")
        x_1 = np.load(f'{save_dir}\\{dt+df_name}\\mat_{PM_mat}_temp_{PM_temp}\\{dt}_2DVnabla_xgeom_2eff_coef.npy')
        fig, axes = plt.subplots(10, 10, figsize=(15, 15))
        i = 0
        for latent_variable in x_1[-1]:
...
for PM_mat in PM_material_list:
    for PM_temp in PM_TEMP_list:
        f_1 = np.load(f'{save_dir}\\{dt+df_name}\\mat_{PM_mat}_temp_{PM_temp}\\{dt}_2DVnabla_fitness_2eff_coef.npy')
        x_1 = np.load(f'{save_dir}\\{dt+df_name}\\mat_{PM_mat}_temp_{PM_temp}\\{dt}_2DVnabla_xgeom_2eff_coef.npy')
        g_1 = np.load(f'{save_dir}\\{dt+df_name}\\mat_{PM_mat}_temp_{PM_temp}\\{dt}_2DVnabla_constraint_2eff_coef.npy')

        check_constraint_last = ~((g_1[-1]>0).sum(axis=1).astype(bool)|(f_1[-1][:,0]<3))
        ...
        # ax.plot(
        #     (-np.array(g_1)[:,:,0]*PA[0]+PA[0]*alpha).flatten(),
        #     (-np.array(g_1)[:,:,0]*PA[0]+PA[0]*alpha).flatten(),
        #         'o', c='k', ms=1, label='All pop.')
        # ax.plot(
        #     -np.array(g_1)[-1,:,0]*PA[0]+PA[0]*alpha,
        #     -np.array(g_1)[-1,:,0]*PA[0]+PA[0]*alpha,
        #         's', c='g', ms=2, label='Last pop.')
        # ax.plot(
        #     -np.array(g_1_)[:,0]*PA[0]+PA[0]*alpha,
        #     -np.array(g_1_)[:,0]*PA[0]+PA[0]*alpha,
        #         '^', c='r', ms=2, label='Last pop.')

        # # ax.set_xlabel('Torque at P_A (Nm)')
        # ax.set_ylabel('Torque at P_A (Nm)')
    ...
    # for Ie in Ie_list:
        ...
        f_1_ok = f_1[-1][check_constraint_last]
        f_1_ok_all[f'{PM_mat}_{PM_temp}'] = f_1_ok
        err = False
        try:
            if check_constraint_last.sum()>4:
                pf_tmp = np.array(calc_pareto_front(f_1_ok))
            else:
                pf_tmp = f_1_ok
        ...
        if show_figure:
            ax.plot(np.array(f_1_ok)[:,0],
                    np.array(f_1_ok)[:,1],
                    '^', c='r', ms=3, label='Last population')
            ax.set_xlabel(f'PM volume')
            ax.set_ylabel(f'torque')
                ...
                for i in range(pf_tmp.shape[0]):
                    latent_variable = x_1[n][
                        np.where(
                            ...
                            np.array(f_1[-1])==pf_tmp[i][0]
                        )[0][0]]
                    # pf_tmp_conditions.append(f_1[-1][n][
                    #     np.where(
...
for PM_temp in PM_TEMP_list:
    ...
    for PM_mat in PM_material_list:
        ...
        if pf_tmp.shape[0]==1:
            plt.plot(pf_tmp_all[key][:,0], -pf_tmp_all[key][:,1]*PA[0], 'o', label=key)
        else:
            if PM_mat.startswith('NMX'):
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[1,i/5,i/5])
                plt.plot(pf_tmp_all[key][:,0], -pf_tmp_all[key][:,1]*PA[0], linewidth=1, label=key, color=[i/5,0,0])
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[i/9,0,0])
                i += 1
            elif PM_mat.startswith('R'):
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[j/5,j/5,1])
                plt.plot(pf_tmp_all[key][:,0], -pf_tmp_all[key][:,1]*PA[0], linewidth=1, label=key, color=[0,0,j/3])
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[0,0,j/9])
                j += 1
            else:
                print('err')
    # plt.title(PM_mat)
...
for PM_temp in PM_TEMP_list:
    ...
    for PM_mat in PM_material_list:
        ...
        V_pm = pf_tmp_all[key][:,0]/100*V
        T = -pf_tmp_all[key][:,1]*PA[0]
        T_ver_PMV[key] = T/V_pm

        PM_data_int = evaln._interpolate(PM_temp, PM_data[PM_mat])
        PM_charac[key] = PM_data_int

    # k = 0
    # for PM_mat in PM_material_list:
    #     key = f'{PM_mat}_{PM_temp}'
    #     plt.plot(np.ones(T_ver_PMV[key].shape[0])*k, T_ver_PMV[key],'ko')
    #     k += 1
    fig, ax = plt.subplots(figsize=(12/2.54,6/2.54))
    ax.boxplot([T_ver_PMV[f'{PM_mat}_{PM_temp}'] for PM_mat in PM_material_list], labels=PM_material_list,sym="")
    ax.set_xticklabels(ax.get_xticklabels(), rotation=90, fontsize=10)
    ax.tick_params(axis='y', labelsize=10)
    # ax.set_title(PM_temp)
    ax.set_ylabel('Torque per PM volume (Nm/cm3)', fontsize=10.5)
    ax.set_ylim([2000, 9000])
    # fig.tight_layout()
    plt.show()

    fig, ax = plt.subplots(figsize=(10/2.54,6/2.54))
    x = [PM_charac[f'{PM_mat}_{PM_temp}']['Remanence'] for PM_mat in PM_material_list]
    y = [T_ver_PMV[f'{PM_mat}_{PM_temp}'].mean() for PM_mat in PM_material_list]
    y_err = [T_ver_PMV[f'{PM_mat}_{PM_temp}'].std() for PM_mat in PM_material_list]
    ax.errorbar(x[:6], y[:6], yerr=y_err[:6], fmt='o', label='Nd', capsize=3, ecolor='k', color='red')
    ax.errorbar(x[6:], y[6:], yerr=y_err[6:], fmt='o', label='SmCo', capsize=3, ecolor='k', color='blue')

    x_all = []
    y_all = []
    for PM_mat in PM_material_list:
        y_ = list(T_ver_PMV[f'{PM_mat}_{PM_temp}'])
        x_ = PM_charac[f'{PM_mat}_{PM_temp}']['Remanence']
        x_all.extend([x_]*len(y_))
        y_all.extend(y_)
    coeffs = np.polyfit(x_all, y_all, 1)
    x_cal = np.linspace(min(x_all)*0.9, max(x_all)*1.1, 100)
    approx_y = np.polyval(coeffs, x_cal)
    ax.plot(x_cal, approx_y, '--', color='green')
    r = np.corrcoef(x_all, y_all)[0, 1]
    R2 = r2_score(x_cal, approx_y)
    print(coeffs)
    # ax.text(x_cal[0], approx_y[0]*1.1, f"Corr.:{R2:.03f}", fontsize=10, c='g')
    ax.tick_params(axis='x', labelsize=10)
    ax.tick_params(axis='y', labelsize=10)
    ax.set_xlabel('Remanence (T)', fontsize=10.5)
    ax.set_ylabel('Torque per PM volume (Nm/cm3)', fontsize=10.5)
    ax.set_ylim([2000, 9000])
    ...
    fig, ax = plt.subplots(figsize=(10/2.54,6/2.54))
    x = np.array([PM_charac[f'{PM_mat}_{PM_temp}']['Coercivity'] for PM_mat in PM_material_list])/1000
    y = [T_ver_PMV[f'{PM_mat}_{PM_temp}'].mean() for PM_mat in PM_material_list]
    y_err = [T_ver_PMV[f'{PM_mat}_{PM_temp}'].std() for PM_mat in PM_material_list]
    ax.errorbar(x[:6], y[:6], yerr=y_err[:6], fmt='o', label='Nd', capsize=3, ecolor='k', color='red')
    ax.errorbar(x[6:], y[6:], yerr=y_err[6:], fmt='o', label='SmCo', capsize=3, ecolor='k', color='blue')
    ...
    for PM_mat in PM_material_list:
        y_ = list(T_ver_PMV[f'{PM_mat}_{PM_temp}'])
        x_ = np.array(PM_charac[f'{PM_mat}_{PM_temp}']['Coercivity'])/1000
        x_all.extend([x_]*len(y_))
        y_all.extend(y_)
    coeffs = np.polyfit(x_all, y_all, 1)
    x_cal = np.linspace(min(x_all)*1.1, max(x_all)*0.9, 100)
    approx_y = np.polyval(coeffs, x_cal)
    ax.plot(x_cal, approx_y, '--', color='green')
    r = np.corrcoef(x_all, y_all)[0, 1]
    R2 = r2_score(x_cal, approx_y)
    ...
    ax.tick_params(axis='x', labelsize=10)
    ax.tick_params(axis='y', labelsize=10)
    ax.set_xlabel('Coercivity (kA/m)', fontsize=10.5)
    ax.set_ylabel('Torque per PM volume (Nm/cm3)', fontsize=10.5)
    ax.set_ylim([2000, 9000])
    ax.legend(loc='best', fontsize=10)
    plt.show()
...
for PM_temp in PM_TEMP_list:
    ...
    for PM_mat in PM_material_list:
        ...
        V_pm = f_1_ok_all[key][:,0]/100*V
        T = -f_1_ok_all[key][:,1]*PA[0]
        T_ver_PMV[key] = T/V_pm

    plt.boxplot([T_ver_PMV[f'{PM_mat}_{PM_temp}'] for PM_mat in PM_material_list], labels=PM_material_list)#,sym="")
...
for PM_mat in PM_material_list:
    ...
    for PM_temp in PM_TEMP_list:
        ...
        if pf_tmp.shape[0]==1:
            ...
            if PM_mat.startswith('NMX'):
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[1,i/5,i/5])
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[i/5,0,0])
                plt.plot(pf_tmp_all[key][:,0], -pf_tmp_all[key][:,1]*PA[0], linewidth=1, label=key, color=[i/9,0,0])
                i += 1
            elif PM_mat.startswith('R'):
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[j/5,j/5,1])
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[0,0,j/3])
                plt.plot(pf_tmp_all[key][:,0], -pf_tmp_all[key][:,1]*PA[0], linewidth=1, label=key, color=[0,0,j/9])
                j += 1
            else:
                print('err')
    plt.title(PM_mat)

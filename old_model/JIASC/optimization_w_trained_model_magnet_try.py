...
save_dir = Path('optimization_results') / 'change_PM'
dt = '230526224322'
df_name = '_torque_unequal_const'
# x_1_all = []
pf_tmp_all = {}

f.only_setting(
    Ie_max=300, Vdc=650, TEMP_PM=PM_TEMP_list[0], PM_material=PM_material_list[0],
    torques=[[280, 3000],[55, 11000]], 
    efficiencies=[[50, 3500],[50, 11000]],
)
...
for PM_mat in PM_material_list:
    for PM_temp in PM_TEMP_list:
        ...
        hv_1 = np.load(f'{save_dir}\\{dt+df_name}\\mat_{PM_mat}_temp_{PM_temp}\\{dt}_2DVnabla_hv_2eff_coef.npy')
        cond_1 = np.load(f'{save_dir}\\{dt+df_name}\\mat_{PM_mat}_temp_{PM_temp}\\{dt}_2DVnabla_conditions_2eff_coef.npy')
        loss_1 = np.load(f'{save_dir}\\{dt+df_name}\\mat_{PM_mat}_temp_{PM_temp}\\{dt}_2DVnabla_losses_2eff_coef.npy')
    ...
    for PM_temp in PM_TEMP_list:
        ...
        # for Ie in Ie_list:
            ...
            ax.plot(-np.array(f_1).reshape(s[0]*s[1], s[2])[:,0]*100,
                    ...
                    'o', c='k', ms=1, label='All population')
            ax.plot(-np.array(f_1[-1])[:,0]*100,
                    -np.array(f_1[-1])[:,1]*100,
                    '^', c='g', ms=3, label='Last population')
        f_1_ = f_1[-1]
        f_1_ok = f_1_[check_constraint_last]
        err = False
        try:
            if check_constraint_last.sum()>4:
            ...
            pf_tmp_all[f'{PM_mat}_{PM_temp}'] = pf_tmp
            if show_figure:
                # ax.plot(-pf_tmp[:,0]*100, -pf_tmp[:,1]*100, c='b', linewidth=1)
                ax.plot(-pf_tmp[:,0]*100, -pf_tmp[:,1]*100, c=[2/5,0,0], linewidth=1)
        except:
            err = True
        if pf_tmp.shape[0]==0: err = True
        if show_figure:
            # ax.plot(-np.array(f_1_ok)[:,0]*100,
            #         -np.array(f_1_ok)[:,1]*100,
            #         '^', c='r', ms=3, label='Last population')
            ax.set_xlabel(f'Efficiency at P1 (%)')
            ax.set_ylabel(f'Efficiency at P2 (%)')
            ...
            if err:
                ...
                fig, axes = plt.subplots(res, res, figsize=(10, 10))
                for i in range(pf_tmp.shape[0]):
                # fig, axes = plt.subplots(3, 5, figsize=(8.4, 4))
                # for i in range(15):
                    latent_variable = x_1[n][
                        np.where(
                            # np.array(loss_1_)==pf_tmp[pf_tmp[:,1]<-0.5][i][0]
                            np.array(f_1_)==pf_tmp[i][0]
                        )[0][0]]
                    pf_tmp_conditions.append(cond_1[n][
                        np.where(
                            np.array(f_1_)==pf_tmp[i][0]
...
for PM_temp in PM_TEMP_list:
    ...
    for PM_mat in PM_material_list:
        key = f'{PM_mat}_{PM_temp}'
        if pf_tmp.shape[0]==1:
            ax.plot(-pf_tmp_all[key][:,0]*100, -pf_tmp_all[key][:,1]*100, 'o', label=key)
        else:
            if PM_mat.startswith('NMX'):
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[1,i/5,i/5])
                ax.plot(-pf_tmp_all[key][:,0]*100, -pf_tmp_all[key][:,1]*100, linewidth=1, label=key, color=[i/5,0,0])
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[i/9,0,0])
                i += 1
            elif PM_mat.startswith('R'):
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[j/5,j/5,1])
                ax.plot(-pf_tmp_all[key][:,0]*100, -pf_tmp_all[key][:,1]*100, linewidth=1, label=key, color=[0,0,j/3])
                # plt.plot(pf_tmp_all[key][:,0], pf_tmp_all[key][:,1], linewidth=1, label=key, color=[0,0,j/9])
                j += 1
            else:
                print('err')

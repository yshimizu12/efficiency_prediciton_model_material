...
def result_name(modelname):
    return f'pred_torque_brhc_{modelname}_w_params'
...
def train_step(x1, x2, x3, t1, model, optimizer):
    model.train()
    ...
    optimizer.step()
    return loss1, preds
def valid_step(x1, x2, x3, t1, model):
    model.eval()
    preds = model(x1, x2, x3)
    # t1 = t1.unsqueeze(1)
    # t2 = t2.unsqueeze(1)
...
def main(modelname, typename, pathmodel=None):
    ...
    for t in range(params['times']):
        ...
        for epoch in range(epochs):
            ...
            valid_loss1 = 0.
            # valid_loss2 = 0.
            for (x1, x2, x3, t1) in train_loader:
                x1, x2, x3, t1 = x1.to(device), x2.to(device), x3.to(device), t1.to(device)
                loss, _ = train_step(x1, x2, x3, t1, model, optimizer)
                train_loss1 += loss.item()
                # train_loss2 += loss[1].item()
            train_loss1 /= len(train_loader)
            # train_loss2 /= len(train_loader)
            for (x1, x2, x3, t1) in valid_loader:
                x1, x2, x3, t1 = x1.to(device), x2.to(device), x3.to(device), t1.to(device)
                loss, _ = valid_step(x1, x2, x3, t1, model)

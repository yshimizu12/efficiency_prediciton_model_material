...
for PM_mat in PM_material_list:
    ...
    # for Ie in Ie_list:
        ...
        try:
            ...
            # ax.plot(pf_tmp[:,0], pf_tmp[:,1], c='b', linewidth=1)
            ax.plot(pf_tmp[:,0], pf_tmp[:,1], color=[4/6,0,0], linewidth=1)
        except:
            err = True
        if pf_tmp.shape[0]==0: err = True
        if show_figure:
            ...
            ax.set_xlabel(f'Loss at P1 (W)')
            ax.set_ylabel(f'Loss at P2 (W)')
...
for PM_temp in PM_TEMP_list:
    ...
    ax.set_ylim([2900, 3700])
    ax.set_xlabel('Loss at P1 (W)')
    ax.set_ylabel('Loss at P2 (W)')
    plt.show()

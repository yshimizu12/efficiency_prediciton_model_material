...
class Evaluate:
    ...
    def create_efficiency_map(self, rotor_image_tensor, from_dict=False, Vdc=650):
        if from_dict:
            df_NT_pred_all = self.df_NT_pred_all_dict[str(Vdc)]
        else:
            self.coefs_tmp = self.calc_coef(rotor_image_tensor)
            df_NT_pred_all, _, _ = self._calc_NT(self.coefs_tmp, rotor_image_tensor, Ns=range(1000,14000+1,1000), Ie=self.Iem)
        params_plot = self._create_efficiency_map(
            df_NT_pred_all,
            charac_num=0,
            image_save=False,
    ...
    def create_efficiency_map_all(self, rotor_image_tensor, Iem, Vdc_range):
        if self.coefs_tmp is not None:
            self._calc_NT_all(self.coefs_tmp, rotor_image_tensor, Ns=range(1000,14000+1,1000), Iem=Iem, Vdc_range=Vdc_range)
        else:
            print('calculate coefs first!')
...
#             print(charac_arr[:,0])
    ...
    def _calc_NT_all(self, coefs, rotor_image_tensor, Ns=range(1000,14000+1,1000), Iem=134, Vdc_range=[300,600,900]):
        self.df_NT_pred_all_dict = {}
        for Vdc in Vdc_range:
            self._elec_params_setting(Iem, Vdc)
            Ia_range = np.arange(Iem,0,-15)
            Ia_range = Ia_range*3**0.5
            NT_pred_all = np.array([[self._search_other_current(N, coefs, Ia) for N in Ns] for Ia in Ia_range])
            # NT_pred_all_tmp = NT_pred_all.copy()
            beta_T_base = np.array([self._mtpa_beta_search_other_current(Ia, coefs, 0, 90, 5) for Ia in Ia_range])
            ...
            self.df_NT_pred_all_dict[str(Vdc)] = df_NT_pred_all
            # NT_pred_all_tmp = np.vstack((NT_pred_all_tmp,[[[0, 0, N, 0] for N in Ns]]))
            # loss_pred_all = np.array([[self._loss_calculation(Ia, beta, N, rotor_image_tensor)
            #                         for Ia, beta, N in zip(NT_pred_all_tmp[i,:,0],NT_pred_all_tmp[i,:,1],NT_pred_all_tmp[i,:,2])]
            #                         for i in range(NT_pred_all_tmp.shape[0])])
...
class Optimize:
    ...
    def show_best_result(self, Vdc_range):
        if self.result is None:
            ...
            self.Iem_tmp = self.problem.eval.Iem
            self.Vdc_range = Vdc_range
            self.gain_grad_tmp = np.zeros(self.n_grad)
            self.image_tmp = self.generated_image_best.clone()
            self.problem.eval.create_efficiency_map_all(self.generated_image_best, Iem=self.Iem_tmp, Vdc_range=self.Vdc_range)
            print('completed!')

    # def show_different_result(self, Iem, Vdc, gain_grad):
    def show_different_result(self, Iem, Vdc, gain1, gain2, gain3):
        gain_grad = np.array([gain1, gain2, gain3])
        if (gain_grad==self.gain_grad_tmp).sum()==self.n_grad:
            self._show_different_result(Iem, Vdc)
        elif (gain_grad==np.zeros(self.n_grad)).sum()==self.n_grad:
            self.gain_grad_tmp = gain_grad
            self.image_tmp = self.generated_image_best.clone()
            self._show_different_result(Iem, Vdc)
        else:
            self.gain_grad_tmp = gain_grad
    ...
    def _show_result(self, generated_image, from_dict=False, Vdc=650):
        params_plot = self.problem.eval.create_efficiency_map(generated_image, from_dict=from_dict, Vdc=Vdc)
        ...
        ax[0].axis('off')
        self.problem.eval.create_plot_effciency_map(fig, ax[1], *params_plot)
    ...
    def _show_different_result(self, Iem, Vdc, X=None):
        ...
        if X is None:
            if self.Iem_tmp==Iem:
                ...
                self._show_result(self.image_tmp)
                self.Iem_tmp = Iem
                self.problem.eval.create_efficiency_map_all(self.image_tmp, Iem=self.Iem_tmp, Vdc_range=self.Vdc_range)
        else:
            self.image_tmp = self.problem.Generator(X)
            self._show_result(self.image_tmp)
            self.Iem_tmp = Iem
            self.problem.eval.create_efficiency_map_all(self.image_tmp, Iem=self.Iem_tmp, Vdc_range=self.Vdc_range)
...
class MyProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for evaluate_point in self.evaluate_efficiency_points:
            ...
            cond_tmp.append([Ia_point, beta_point, N_point, T_point])
            loss_tmp.append(loss+(power,))
        cond_tmp = np.array(cond_tmp)
        conditions = {}
        conditions['Ia'] = cond_tmp[:,0]
        conditions['beta'] = cond_tmp[:,1]
        conditions['N'] = cond_tmp[:,2]
        conditions['T'] = cond_tmp[:,3]
        losses = {}
        loss_tmp = np.array(loss_tmp)
        losses['copper loss'] = loss_tmp[:,0]
        losses['iron loss'] = loss_tmp[:,1]
        losses['power'] = loss_tmp[:,2]
        out["F"] = efficiencies

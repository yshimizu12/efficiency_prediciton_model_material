...
def result_name(modelname):
    return f'pred_ironloss_brhc_{modelname}_w_params_hidden_dim_other'
...
def main(modelname, typename, pathmodel=None, hidden_dim_other_list=None, hidden_dim_other2_list=None):
    ...
    def model_name(num, times, h1, h2):
        return str(base_dir / results_dir / name / f'model_{num}_{times}_{h1}_{h2}.pt')
    def save_model(model, num, times, h1, h2):
        torch.save(model, model_name(num, times, h1, h2))
    def save_result(result, num, times, h1, h2):
        # with open(str(base_dir / results_dir / name / f'result_{num}_{times}.csv'), 'w', encoding='Shift_jis') as f:
        with open(str(base_dir / results_dir / name / f'result_{times}_{h1}_{h2}.csv'), 'w', encoding='Shift_jis') as f:
    ...
    for hidden_dim_other in hidden_dim_other_list:
        for hidden_dim_other2 in hidden_dim_other2_list:
            params['hidden_dim_other'] = hidden_dim_other
            params['hidden_dim_other2'] = hidden_dim_other2
            for t in range(params['times']):
                ...
                save_result(results, epoch+1, t, hidden_dim_other, hidden_dim_other2)
...
if __name__=="__main__":
    ...
    parser.add_argument(
        "hidden_dim_other_list",
        default=None,
        type=ast.literal_eval,
    )
    parser.add_argument(
        "hidden_dim_other2_list",
        default=None,
        type=ast.literal_eval,
    )
    parser.add_argument(
    ...
    print(type(args.hidden_dim_other_list), args.hidden_dim_other_list)
    print(type(args.hidden_dim_other2_list), args.hidden_dim_other2_list)

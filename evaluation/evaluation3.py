...
class Evaluate:
    ...
    def _calc_encoded_img(self, image, calc_grad=False):
        ...
        return encoded_img_flux, encoded_img_ironloss
...
class MyProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        PM_volume = self.eval._calculate_magnet_volume(generated_image)
        encoded_img_all = self.eval._calc_encoded_img(generated_image)
        _, _, _, Tmax = self.eval._search(self.required_torque_points[0,1], encoded_img_all[0])
        ...
        # print(efficiencies)
        # out["F"] = efficiencies
        out["F"] = [PM_volume, -Tmax/self.required_torque_points[0][0]]
        # out["F"] = PM_volume/10

        # print([PM_volume, -Tmax])
        ...
        # out["G"] = [
        #     (self.required_torque_points[0][0]*1.05-charac_at_req[0,3])/self.required_torque_points[0][0],
        #     (-self.required_torque_points[0][0]*1.05*1.03+charac_at_req[0,3])/self.required_torque_points[0][0],
        # ]
